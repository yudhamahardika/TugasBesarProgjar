...
def send_file(client_socket, recipient):
    try:
        file_path = input("Masukkan file yang akan dituju: ")
...
def send_message(client_socket, username):
    while True:
        try:
            message = input()
            if message == "file":
                send_file(client_socket, username)
            elif message == 'chat':
...
def main():
    ...
    server_address = ('10.217.16.194', 13375)  # Ganti dengan alamat IP mesin server
    client_socket.connect(server_address)
    username = input("Masukkan nama Anda: ")
    client_socket.send(bytes(username, encoding='utf-8'))
    print("1. masukkan chat \n2. masukkan file")
